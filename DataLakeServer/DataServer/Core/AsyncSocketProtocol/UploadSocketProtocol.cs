    ...
    public class UploadSocketProtocol : BaseSocketProtocol
    {
        ...
        public bool DoUpload()
        {
            ...
            if (m_incomingDataParser.GetValue(ProtocolKey.DirName, ref dirName) & m_incomingDataParser.GetValue(ProtocolKey.FileName, ref fileName))
            {
                ...
                CoreService.Logger.Info("Start upload file: " + fileName);
                ...
                if (File.Exists(fileName))
                {
                    ...
                    else
                    {
                        ...
                        CoreService.Logger.Error("Start upload file error, file is in use: " + fileName);
                    }
                }
                else
                {
        ...
        public bool DoEof()
        {
            ...
            {
                CoreService.Logger.Info("End upload file: " + m_fileName);
                m_fileStream.Close();
                m_fileStream = null;
                m_fileName = "";
                m_outgoingDataAssembler.AddSuccess();

...
namespace DataServer
{
    public class DownloadSocketProtocol : BaseSocketProtocol
    {
        ...
        public override bool ProcessCommand(byte[] buffer, int offset, int count) //处理分完包的数据，子类从这个方法继承
        {
            ...
            {
                CoreService.Logger.Error("Unknow command: " + m_incomingDataParser.Command);
                return false;
            }
        }
        ...
        public bool DoDir()
        {
            ...
            {
                if (parentDir == "")
                    parentDir = CoreService.FileDirectory;
                else
                    parentDir = Path.Combine(CoreService.FileDirectory, parentDir);
        ...
        public bool DoFileList()
        {
            ...
            {
                if (dirName == "")
                    dirName = CoreService.FileDirectory;
                else
                    dirName = Path.Combine(CoreService.FileDirectory, dirName);
        ...
        public bool DoDownload()
        {
                ...
                & m_incomingDataParser.GetValue(ProtocolKey.FileSize, ref fileSize) & m_incomingDataParser.GetValue(ProtocolKey.PacketSize, ref packetSize))
            {
                ...
                CoreService.Logger.Info("Start download file: " + fileName);
                ...
                if (File.Exists(fileName))
                {
                    ...
                    else
                    {
                        ...
                        CoreService.Logger.Error("Start download file error, file is in use: " + fileName);
                    }
                }
                else
                {
        ...
        public override bool SendCallback()
        {
            ...
            if (m_fileStream != null)
            {
                ...
                else
                {
                    ...
                    {
                        CoreService.Logger.Info("End download file: " + m_fileName);
                        m_fileStream.Close();
                        m_fileStream = null;
                        m_fileName = "";
                        m_sendFile = false;

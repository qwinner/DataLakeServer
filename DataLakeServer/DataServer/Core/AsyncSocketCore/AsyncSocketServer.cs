...
namespace DataServer
{
    public class AsyncSocketServer
    {
        ...
        public void Start(IPEndPoint localEndPoint)
        {
            ...
            CoreService.Logger.InfoFormat("Start listen socket {0} success", localEndPoint.ToString());
        ...
        void AcceptEventArg_Completed(object sender, SocketAsyncEventArgs acceptEventArgs)
        {
            ...
            {
                CoreService.Logger.ErrorFormat("Accept client {0} error, message: {1}", acceptEventArgs.AcceptSocket, E.Message);
                CoreService.Logger.Error(E.StackTrace);  
            }            
        }

        private void ProcessAccept(SocketAsyncEventArgs acceptEventArgs)
        {
            CoreService.Logger.InfoFormat("Client connection accepted. Local Address: {0}, Remote Address: {1}",
            ...
            {
                CoreService.Logger.ErrorFormat("Accept client {0} error, message: {1}", userToken.ConnectSocket, E.Message);
                CoreService.Logger.Error(E.StackTrace);                
            }            
        ...
        void IO_Completed(object sender, SocketAsyncEventArgs asyncEventArgs)
        {
            ...
            {
                CoreService.Logger.ErrorFormat("IO_Completed {0} error, message: {1}", userToken.ConnectSocket, E.Message);
                CoreService.Logger.Error(E.StackTrace);
            }                     
        }
        ...
        private void ProcessReceive(SocketAsyncEventArgs receiveEventArgs)
        {
            ...
            if (userToken.ReceiveEventArgs.BytesTransferred > 0 && userToken.ReceiveEventArgs.SocketError == SocketError.Success)
            {
                ...
                {
                    CoreService.Logger.WarnFormat("Illegal client connection. Local Address: {0}, Remote Address: {1}", userToken.ConnectSocket.LocalEndPoint, 
        ...
        private void BuildingSocketInvokeElement(AsyncSocketUserToken userToken)
        {
            ...
            {
                CoreService.Logger.InfoFormat("Building socket invoke element {0}.Local Address: {1}, Remote Address: {2}",
        ...
        public void CloseClientSocket(AsyncSocketUserToken userToken)
        {
            ...
            string socketInfo = string.Format("Local Address: {0} Remote Address: {1}", userToken.ConnectSocket.LocalEndPoint,
                userToken.ConnectSocket.RemoteEndPoint);
            CoreService.Logger.InfoFormat("Client connection disconnected. {0}", socketInfo);
            try
            {
            ...
            {
                CoreService.Logger.ErrorFormat("CloseClientSocket Disconnect client {0} error, message: {1}", socketInfo, E.Message);
            }
